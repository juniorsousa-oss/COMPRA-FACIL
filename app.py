...
def save_product(name,cat,unit,price=0,qty=1):
    ...
    if r:
        p=r[0];old=num(p.get("ultimo_preco"));prices=[x for x in(old,price) if x>0];payload.update(preco_medio=sum(prices)/len(prices) if prices else num(p.get("preco_medio")),menor_preco=min(prices) if prices else num(p.get("menor_preco")),maior_preco=max(prices) if prices else num(p.get("maior_preco")),quantidade_compras=int(p.get("quantidade_compras") or 0)+1);db("produtos","PATCH",params={"id":f"eq.{p['id']}"},data=payload);return p["id"]
...
def new_product_dialog(products):
    ...
    with st.form("new_product_form"):
        name=st.text_input("Nome",placeholder="Ex.: Arroz branco")
        cat=st.selectbox("Categoria",cats)
        unit=st.text_input("Unidade",value="un.",placeholder="un., kg, L, pct...")
        if st.form_submit_button("Cadastrar produto",type="primary",use_container_width=True):
            ...
            else:save_product(name,cat,unit.strip(),0,1);st.success("Produto cadastrado.");st.rerun()
...
def import_history_excel(uploaded,when,budget):
    df=pd.read_excel(uploaded,dtype=object);df.columns=[norm(c) for c in df.columns];find=lambda names:next((norm(n) for n in names if norm(n) in df.columns),None);cp=find(["PRODUTO","Produto","Nome"]);cu=find(["VALOR UNITÁRIO","Valor Unitário","Preço Unitário","Preço"]);cq=find(["QNT","Quantidade","Qtd","Qtde"]);cl=find(["ÚLTIMO VALOR","Ultimo Valor","Último Preço","Ultimo Preco"])
    ...
    real=sum(x["qty"]*x["price"] for x in rows);estimated=sum(x["qty"]*(x["last"] if x["last"]>0 else x["price"]) for x in rows);c=db("compras","POST",data={"orcamento":num(budget),"valor_estimado":estimated,"valor_real":real,"saldo":num(budget)-real,"quantidade_itens":len(rows),"data_compra":f"{when.isoformat()}T12:00:00+00:00"})[0];products=get_products();items=[]
    for x in rows:
        existing=next((p for p in products if norm(p.get("nome"))==norm(x["name"])),None);cat=existing.get("categoria","Mercearia") if existing else "Mercearia";unit=existing.get("unidade","un.") if existing else "un.";previous=x["last"] if x["last"]>0 else (num(existing.get("ultimo_preco")) if existing else 0);pid=save_product(x["name"],cat,unit,x["price"],x["qty"]);items.append({"compra_id":c["id"],"produto_id":pid,"nome_produto":x["name"],"quantidade":x["qty"],"unidade":unit,"preco_estimado":previous,"preco_unitario":x["price"],"valor_total":x["qty"]*x["price"],"ultimo_preco":previous,"variacao_preco":x["price"]-previous,"confirmado":True})
    db("itens_compra","POST",data=items);clear();return len(rows),real
...
estimated=sum(num(x.get("quantidade"))*num(x.get("preco_estimado")) for x in current);real=sum(num(x.get("quantidade"))*num(x.get("preco_unitario")) for x in current if x.get("confirmado"));done=sum(1 for x in current if x.get("confirmado"));cols=st.columns(4)
...
with buy:
    ...
    with b:
        if st.button("Importar lista padrão",use_container_width=True,key="import_standard"):
            ...
            elif total:st.info("Os produtos recorrentes já estão na lista atual.")
            else:st.info("Ainda não há produtos recorrentes no histórico.")
            st.rerun()
    if not current:st.markdown('<div class="empty"><strong>Sua lista está vazia.</strong><br>Adicione um produto cadastrado ou importe a lista padrão.</div>',unsafe_allow_html=True)
    for item in current:
        ...
        with a:cls="product-name done" if ok else "product-name";st.markdown(f'<div class="{cls}">{item["nome_produto"]}</div>',unsafe_allow_html=True);st.markdown(f'<div class="muted">{item.get("categoria","Mercearia")} · {num(item.get("quantidade")):g} {item.get("unidade","un.")}</div>',unsafe_allow_html=True)
...
with prod:
    ...
    st.info("O cadastro manual e a importação usam somente: Nome, Categoria e Unidade. Preço e quantidade serão preenchidos quando houver uma compra.")
    with st.expander("📥 Importar lista Excel",expanded=False):
        st.write("O arquivo deve conter obrigatoriamente estas 3 colunas: **NOME**, **CATEGORIA** e **UNIDADE**.")
            ...
            try:
                ...
                if error:st.error(error)
                    ...
                    if imported:
                        ...
                        if st.button("Cadastrar produtos novos",type="primary",use_container_width=True,key="import_confirm"):
...
with hist:
    st.subheader("Histórico de compras");st.markdown("### Excel");st.caption("Padrão: PRODUTO · VALOR UNITÁRIO · QNT · ÚLTIMO VALOR · VALOR TOTAL.");all_rows=history_rows()
    if all_rows:
    ...
    with st.expander("Importar histórico do Excel",expanded=False):
        st.caption("Obrigatórias: PRODUTO, VALOR UNITÁRIO e QNT. ÚLTIMO VALOR é opcional; VALOR TOTAL é recalculado.");import_date=st.date_input("Data da compra importada",value=date.today(),key="history_import_date");import_budget=st.number_input("Orçamento da compra",min_value=0.,value=0.,step=10.,format="%.2f",key="history_import_budget");hist_file=st.file_uploader("Escolher Excel do histórico",type=["xlsx","xlsm"],key="excel_history")
        if hist_file is not None:
            try:
                ...
                if st.button("Importar esta compra para o histórico",type="primary",use_container_width=True,key="history_import_confirm"):
                    n,total=import_history_excel(hist_file,import_date,import_budget);st.success(f"{n} itens importados. Total real: {money(total)}");st.rerun()
            except Exception as e:st.error(f"Erro na importação: {e}")
    st.divider()
    ...
    for p in history:
        ...
        with st.expander(f"{date_text} · {money(p.get('valor_real'))} · {p.get('quantidade_itens',0)} itens"):
            a,b,c,d=st.columns(4);a.metric("Orçamento",money(p.get("orcamento")));b.metric("Estimado",money(p.get("valor_estimado")));c.metric("Real",money(p.get("valor_real")));d.metric("Saldo",money(p.get("saldo")));items=db("itens_compra",params={"select":"*","compra_id":f"eq.{p['id']}","order":"id.asc"})
