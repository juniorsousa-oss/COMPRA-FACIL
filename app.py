...
_old_q='''            q=st.number_input(\"Qtd\",min_value=.001,value=num(item.get(\"quantidade\")) or 1.,step=1.,key=f\"q{item['id']}\",disabled=ok)
            if q!=num(item.get(\"quantidade\")) and not ok: edit_item(item[\"id\"],quantidade=q); st.rerun()
'''
_new_q='''            q=st.number_input(\"Qtd\",min_value=.001,value=num(item.get(\"quantidade\")) or 1.,step=1.,key=f\"q{item['id']}\",disabled=False)
            if q!=num(item.get(\"quantidade\")):
                edit_item(item[\"id\"],quantidade=q,confirmado=ok)
                st.rerun()
'''
_source=_source.replace(_old_q,_new_q,1)
