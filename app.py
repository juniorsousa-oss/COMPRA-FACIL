...
# Permitir orçamento para a próxima compra.
_source = _source.replace(
    'st.session_state["open_add_after_budget" if action=="add" else "open_standard_after_budget"]=True',
    'st.session_state["open_add_after_budget" if action=="add" else ("open_next_after_budget" if action=="next" else "open_standard_after_budget")]=True',
    1,
)
_source = _source.replace(
...
# Mostrar próxima lista quando a lista atual estiver vazia.
...
# Adicionar botão Próxima lista aos cards.
...
# Cadastro de produto: consulta o banco no momento do cadastro.
...
_source = re.sub(
    r'@st\.dialog\("Adicionar novo produto"\)\ndef new_product_dialog\(products\):.*?\ndef import_products_excel',
    _new_product_dialog + '\ndef import_products_excel',
    _source,
    count=1,
    flags=re.S,
